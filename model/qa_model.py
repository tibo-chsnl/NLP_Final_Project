...
class RNNEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
        ...
        Args:
            ...
            mask: Optional Tensor of shape (batch_size, seq_len) boolean mask where True/1 indicates valid tokens.
        Returns:
...
class BiDAFAttention(nn.Module):
    ...
    def __init__(self, hidden_dim: int):
        ...
        # So the concatenated input to the linear layer is 3 * (2 * rnn_hidden_dim) = 6 * rnn_hidden_dim
        self.W = nn.Linear(3 * hidden_dim, 1, bias=False)
        
    def forward(self, context: torch.Tensor, query: torch.Tensor, context_mask: torch.Tensor = None, query_mask: torch.Tensor = None) -> torch.Tensor:
        """
        Args:
            context: Tensor of shape (batch_size, c_len, hidden_dim)
            query: Tensor of shape (batch_size, q_len, hidden_dim)
            context_mask: Optional boolean Tensor of shape (batch_size, c_len) where 1 indicates valid context token.
            query_mask: Optional boolean Tensor of shape (batch_size, q_len) where 1 indicates valid query token.
        Returns:
            Tensor of shape (batch_size, c_len, 4 * hidden_dim) representing the query-aware context representation.
        """
        ...
        G = torch.cat([
            ...
            context * c2q,
            context * q2c
        ], dim=-1) # (batch_size, c_len, 4 * hidden_dim)
...
class QAModel(nn.Module):
    ...
    """
    def __init__(self, vocab_size: int, embedding_dim: int = 100, hidden_dim: int = 128, dropout: float = 0.2):
        super().__init__()
    ...
    def forward(self, context: torch.Tensor, query: torch.Tensor, context_mask: torch.Tensor = None, query_mask: torch.Tensor = None) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Args:
